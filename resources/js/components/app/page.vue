...
>
  ...
  <div class="tags__list">
    <span
      ...
      v-on:click="pickNewItem(item.id, 'tag')"
    >{{ item.name }}</span>
  </div>
</div>
...
>
  ...
  <div class="projects__list">
    <span
      ...
      v-on:click="pickNewItem(item.id, 'project')"
    >{{ item.title }}</span>
  </div>
</div>
...
<div id="page" class="site" >
  ...
  >
    <div class="ic__editor drop"></div>
      ...
      <div
        v-if="hasTag(group.tags, item.id) && item.title === 'tag'"
        v-for="group in projects.groups"
      ...
      >
        ...
        >{{item.linkTitle}}</a>
          ...
          >
            ...
            <button class="ic__editor drag" v-on:click="pickItem(item.title, item.id, item.type_id, item.link, item.tag_id, index)"></button>
            ...
            >
              ...
              <input
                ...
                >
              <button class="ic__editor trash" v-on:click="item.link = '', item.linkTitle = ''"></button>
  ...
  methods: {
    ...
    getProjects() {
     ...
     hasTag(obj, value) {
      ...
      pickNewItem: function (value, title) {
      ...
      //   // console.log('itemId: ' + this.itemId + ' - itemType: ' + this.itemType + ' - tagId: ' + this.tagId + ' - elemIndex: ' + this.elemIndex)
      // },
      pickItem(title, id, type, link, tagid, index) {
          this.itemTitle = title,
          this.itemId = id,
          this.itemType = type,
          this.itemLink = type,
      ...
      dropItem(index) {
        ...
        // if(this.itemId != '') {
          ...
          if(this.elemIndex === '') {
            ...
            if(this.itemTitle == 'project') {
              arr.splice(index + 1, 0, {'title': this.itemTitle, 'id': this.itemId, 'type_id': '20', 'link': this.itemLink, 'link_title': this.linkTitle })
            }
            if(this.itemTitle == 'tag') {
              arr.splice(index + 1, 0, {'title': this.itemTitle, 'id': this.itemId, 'type_id': '20', 'link': this.itemLink, 'link_title': this.linkTitle })
            }
            // arr.splice(index + 1, 0, {project:{'id': this.itemId, 'type_id': 20}}),
            this.getProjects(),
            this.cancelDrop()
          } else {
      ...
      dropItemHead() {
        ...
        if(this.elemIndex == '') {
          if(this.itemTitle == 'project') {
            arr.splice(0, 0, {'title': this.itemTitle, 'id': this.itemId, 'type_id': '20', 'link': this.itemLink, 'link_title': this.linkTitle })
          }
          if(this.itemTitle == 'tag') {
            arr.splice(0, 0, {'title': this.itemTitle, 'id': this.itemId, 'type_id': '20', 'link': this.itemLink, 'link_title': this.linkTitle })
          }
          // if(this.itemId != '') {
          //   arr.splice(0, 0, {'title': 'project', 'id': this.itemId, 'type_id': 20})
          // }
          // if(this.tagId != '') {
          //   arr.splice(0, 0, {'title': 'tag', 'id': this.tagId, 'type_id': 20})
          // }
